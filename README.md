...
集中维护 Lethink CMS 开发相关的 Agent Skills，供团队通过 `npx skills` 安装到 ZCode、Codex 等编程助手。
...
准备 Node.js、npm（包含 npx）和 Git。仓库推送后，将下方的“你的组织/lethink-skills”替换为实际 GitHub 仓库名：

```bash
npx skills@latest add 你的组织/lethink-skills --skill create-lethink-dynamic-component --agent zcode --global
```

公司 GitLab、其他 Git 服务或 SSH 地址也可以直接使用实际克隆地址：

```bash
npx skills@latest add "你的实际Git仓库克隆地址" --skill create-lethink-dynamic-component --agent zcode --global
```
...
```bash
npx skills@latest add 你的组织/lethink-skills --skill create-lethink-dynamic-component --agent codex --global
```
...
本项目已初始化本地 Git 仓库，初始分支为 main。创建一个空的远程仓库后，在这里执行：

```bash
git add README.md .gitignore skills
git commit -m "初始化 Lethink 技能仓库"
git remote add origin "你的实际Git仓库克隆地址"
git push -u origin main
```

如果远程仓库已包含提交，先按团队 Git 流程对齐历史。推送完成后，将实际仓库地址填入本文安装示例并告知团队。
