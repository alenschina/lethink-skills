...
默认仍只交付 `unit_json` 和动态 HTML，沿用原 CSS、JS、`deps_json`。本案例曾获准补充公共能力，这不代表所有环境都已部署，也不意味着每次任务都要编写提交脚本或修改后端。

优先查当前代码和相邻组件：

| 检查位置 | 要确认的行为 |
| --- | --- |
| 前端 `src/utils/lethink-dynamic-runtime.js`，搜索 `initConfiguredLeadForm` | `cfg.form` 分支负责提交、验证码和状态反馈 |
...
| `ConfiguredLeadCaptchaService.php`、`app/controller/api/CustomerLead.php`、`config/allowlist.php` | 图片验证码及公开提交入口是否存在且可访问 |
| `app/service/component/ComponentService.php`、`ComponentPageActivationV3Service.php` | 保存、激活时识别表单，避免自动补入列表数据源和生成内容资源 |

现有短信表单（例如 181）可供了解旧流程，但短信接口不能直接当作图片验证码接口。只具备 `configuredSubmit` 也不等于具备图片验证码功能。

若能力齐备，直接复用；若缺失，先说明需要补充哪部分、影响范围和交付限制。已有明确授权时补齐最小公共能力，否则不要声称仅靠 JSON、HTML 就能实现服务端校验。
...
| `lead_form.default.captcha.type: image` | 服务端强制校验图片验证码；不需要验证码时省略 `captcha`，不虚构 `none` 或 `sms` 类型 |
...
- 验证码输入（本例 `name="vcode"`）不放进业务 `fields`，通过独立参数提交。
...
data-lethink-dynamic='{
  ...
  "form": {
    ...
    "captcha_image": "[data-lethink-captcha]",
    "captcha_input": ".vcode-text",
...
`schema` 是对象模板，外面不加双引号。CMS 渲染后检查 `form.schema.fields` 是否仍为数组。原稿表单、图片、验证码输入和提示节点应分别匹配选择器；选择器在当前组件根节点或表单内解析，避免依赖全局唯一 ID。状态提示使用 `role="status"`、`aria-live="polite"`；提交按钮初始 `disabled`，由公共运行时在准备好后启用。

`execute_path` 在此用于解析 API 主机及 `/cmsapi`、`/cmsapi2` 前缀，表单不会调用 execute 查询列表。当前运行时直接发送 `application/x-www-form-urlencoded` POST：

| 控制器路径（以 `/cmsapi` 为例） | 请求体 | 成功响应 |
| --- | --- | --- |
| `/cmsapi/api.CustomerLead/formCaptcha` | `site_id`、`component_id` | `code: 0`，`data` 含 `id`、PNG data URL `image`、`expires_in` |
| `/cmsapi/api.CustomerLead/configuredSubmit` | `site_id`、`component_id`、`data`（业务字段 JSON 字符串）、开启图片验证码时的 `captcha_id` 与 `captcha_code`、来源信息 | `code: 0` 且 `data.id` 为新留资记录 ID |
...
在 CMS 中录入 JSON、HTML，沿用原 CSS、JS 和依赖，挂到测试站点页面并保存。在现有“留资管理”（前端视图 `content/lead`）核对记录和站点权限。此表单不依赖 `cat:product`、`cat:news`，不能用添加文章标签来解决留资入口不可见。

## 验证码与预览的关键行为

当前图片验证码实现为 5 位、不区分大小写、有效期 300 秒；挑战绑定站点和组件，答案摘要只在 Redis。进入消费校验后，无论成功或失败都原子消费；字段校验在验证码消费之前，字段提前拒绝不等于验证码已被消费。当前获取限制是每 IP 每分钟最多 40 次，服务端需要 GD 和 Redis；参数以实际部署版本为准。

公共运行时处理加载、点击/键盘刷新、过期、忽略旧刷新响应、提交期间禁用按钮和防止并发重复 POST。成功后重置输入及自定义下拉显示；失败保留业务输入，并刷新验证码。客户端防重复点击不等于服务端业务幂等，提交超时时需先确认是否入库。

预览判断要区分表单与列表：带 `hooks.previewMode`、无站点或显式静态标志时，表单不取验证码、不提交；缺少组件上下文也禁用。站点列表预览可能仍要读取真实接口，不能为了禁止表单提交而恢复全局 `hooks.previewMode` 静态化。真实提交应在带有效上下文且未被标记为静态/表单预览的测试页面验证。
...
需要补公共能力时检查所有实际入口；各副本可能已有差异，应移植所需分支，不能用一份整文件覆盖其他副本。发布页还可能由 `DynamicRuntimeAssetService` 选取站点或全局 `sys_file` 中路径为 `abc/components/static/js/lethink-dynamic-runtime.js` 的文件中心/CB/CDN 资源。仅提交仓库文件不足以证明站点升级；应在 Network 打开实际加载脚本确认 `initConfiguredLeadForm` 和 `cfg.form` 分支。
...
3. 验证实际运行时：初始禁用、验证码加载与刷新、隐藏省份必填、空邮箱允许、重复点击仅一次 POST、失败保留输入、成功重置、同页实例互不重置，以及表单预览零真实请求。补公共能力时回归站点列表预览仍能取数。
4. 在隔离测试环境走真实控制器和存储：错误、过期、重放、跨站点和跨组件验证码应拒绝；有效请求返回新记录 ID，留资管理可见字段和正确站点。模拟 fetch 成功不能代替入库验证。不要为方便测试向生产代码加入验证码绕过入口。
5. 区分已完成的静态校验、本地运行时/模拟请求、真实后端联调、CMS 挂载发布验证；缺少哪一层就明确列出。核对原 CSS、JS 沿用情况，以及实际发布页资源版本。

排查时：无 execute 请求本身是正常的，先找两个 CustomerLead 请求；按钮一直禁用时检查实际运行时、预览标志、上下文和验证码状态；提交成功但后台列为空时先检查 `phone_field` 与备注标签映射。
