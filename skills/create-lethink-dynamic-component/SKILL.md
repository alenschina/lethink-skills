...
description: 为 Lethink CMS 创建或排查动态组件，默认代码输出仅为 unit_json 和动态 HTML，沿用已有 JS、CSS。适用于基于静态 HTML 或业务需求选择 create_package 或 existing_api、接入 data-lethink-dynamic、配置内容管理入口，以及诊断请求、字段绑定、菜单和链接问题。
---
...
CSS 和已有 JS 属于输入及运行依赖，默认原样沿用。只在数据接入或交互排查确有需要时读取相关实现，不把整套样式和脚本开发纳入任务。缺少静态 HTML 时也只按需求及现有样式约定生成 HTML；数据请求与动态交互复用现有运行时。若确需修改 CSS、JS，说明原因和最小范围；除非用户已明确要求，否则将其列为需另行处理的事项。
...
默认代码交付仅包含完整 `unit_json` 和匹配的动态 HTML。随附后台配置和验证步骤（包含组件标签、数据维护入口、数据作用域），以及验证结果。必要时列明沿用的 CSS、JS 引用即可，无需输出其代码。分类 ID、详情地址等必须由用户填写的配置应单独列明，不用虚构 ID 或 0 代替。
