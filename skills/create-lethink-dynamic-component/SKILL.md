...
CSS 和已有 JS 属于输入及运行依赖，默认原样沿用。只在数据接入或交互排查确有需要时读取相关实现，不把整套样式和脚本开发纳入任务。缺少静态 HTML 时也只按需求及现有样式约定生成 HTML；数据请求与动态交互优先复用现有运行时。若现有运行时缺少必要能力，先说明缺口和最小范围；用户明确只处理当前组件并授权补充时，可将最小组件级 JS 作为第三项代码产物，避免改动公共运行时，具体阅读 [组件级表单运行时](references/component-level-form-runtime.md)。不要同时保留功能重叠的原稿 JS、公共脚本和组件脚本。一次任务中的授权不代表今后每个组件都默认需要或允许新增 JS。
...
3. 写配置前先验证真实来源。搜索前后端实现或查询接口注册表，列表确认接口 `path`、参数、响应样例、`list_path`、站点过滤方式和维护入口；表单确认提交控制器、服务端字段契约、验证码策略、存储映射，以及浏览器端提交能力实际由公共运行时还是组件 JS 承载。不要复用 `component_runtime_*` 之类页面实例生成的接口。
...
4. 阅读 [unit_json 契约](references/unit-json-contract.md)，只加载当前模式所需的模板：
   - `configured_lead_form`：参考 [字段模板](references/configured-lead-form-template.json) 和 [配套 HTML](references/configured-lead-form-template.html)，根据原稿字段调整。只有确认当前组件需要自行承载提交行为时，才继续阅读 [组件级表单运行时](references/component-level-form-runtime.md)。
...
- 表单的 `fields[].key` 与 HTML 控件 `name` 对齐。服务端按源组件当前启用的 `lead_form` 校验；浏览器和页面实例的 schema 不能覆盖服务端策略。验证码独立提交，不混入业务字段。业务允许登录同款点选时优先核对并复用现有验证码能力；先确认发布页浏览器端实现和最终提交复核已支持，不能只改 JSON 类型。组件级实现必须限定在带 `form` 配置的当前根节点、具备重复初始化保护，并从 CMS 挂载上下文读取站点和组件标识。
...
默认代码交付仅包含完整 `unit_json` 和匹配的动态 HTML。随附后台配置和验证步骤（包含适用的组件标签、内容或留资管理入口、数据作用域），以及验证结果。必要时列明沿用的 CSS、JS 引用即可，无需输出其代码。只有现有运行时无法完成需求、用户已授权且范围明确时，才额外交付最小组件级 JS，并说明它替代了哪段公共或原稿行为。分类 ID、详情地址等必须由用户填写的配置应单独列明，不用虚构 ID 或 0 代替。
